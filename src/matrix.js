var Matrix = function (width, height) {
  height = height || width;
  var matrix = Object.create(Matrix.prototype);
...
Matrix.prototype.traverse = function (fn, options) {
  ...
  // Looping
  loop[opt.col.comp](opt.col.start, opt.col.end, opt.col.inc, function (y) { // body of outer
    // get extra row options
    addRowOptions(opt.row, this[y]);
    loop[opt.row.comp](opt.row.start, opt.row.end, opt.row.inc, function (x) {
      fn(this[y][x], y, x, this);
    }.bind(this)); // body of inner
  }.bind(this));
};
